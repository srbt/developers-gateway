...
setup() {
  ...
  docker build --build-arg NODES_PROTOCOL=https -t "${IMAGE_REVERSE_PROXY}" reverse-proxy
  mkdir "${TEST_CONTEXT}/certs"
  docker run --rm -u "$(id -u):$(id -g)" -v "${PWD}/${TEST_CONTEXT}/certs:/certs" "${IMAGE_TOOLBOX}" /usr/local/bin/createCa.sh
  docker run -d --network "${TEST_NETWORK}" --hostname "node.backoffice.in.application.com" --name "${CON_BACKOFFICE}" "${IMAGE_BACKOFFICE}"
  docker run -d --network "${TEST_NETWORK}" --network-alias "backoffice.in.application.com" --hostname "reverse-proxy.in.application.com" --name "${CON_REVERSE_PROXY}" -v "${PWD}/${TEST_CONTEXT}/certs:/certs" "${IMAGE_REVERSE_PROXY}"
...
@test "Backoffice container is running" {
  ...
  run docker run --rm --network "${TEST_NETWORK}" "${IMAGE_TOOLBOX}" curl -s -o /dev/null -w "%{http_code}" "http://node.backoffice.in.application.com:8080/status"
  assert_success
  assert_output '200'
}
...
@test "Reverse proxy is working" {
  ...
  run docker run --rm --network "${TEST_NETWORK}" -v "${PWD}/${TEST_CONTEXT}/certs:/certs" "${IMAGE_TOOLBOX}" curl -s -o /dev/null -w "%{http_code}" -v --cacert /certs/myCA.pem "https://backoffice.in.application.com/status"
  assert_success
